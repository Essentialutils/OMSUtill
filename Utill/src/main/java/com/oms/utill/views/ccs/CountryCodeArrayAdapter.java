...
public class CountryCodeArrayAdapter extends ArrayAdapter<Country> {
    ...
    CountryCodeArrayAdapter(Context ctx, List<Country> countries, CountryCodeSelect picker) {
        super(ctx, 0, countries);
        mCountryCodeSelect = picker;
    ...
    private void setData(Country country, ViewHolder viewHolder) {
        ...
        if (mCountryCodeSelect.isHideNameCode()) {
        ...
        if (mCountryCodeSelect.isHidePhoneCode()) {
            viewHolder.tvCode.setVisibility(View.GONE);
        } else {
        ...
        if (color != mCountryCodeSelect.getDefaultContentColor()) {
